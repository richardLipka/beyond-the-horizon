...
(function (HL) {
  ...
  function mount(container, app) {
    ...
    function renderChart(root, state, result, object) {
      ...
      HL.ReadOut.attach(root, {
        ...
        sample: (px) => {
          ...
          return {
            x: X(distance),
            // Y() si spodni mez orizne sam, horni se orezava tady: za mezi
            // dohledu vyjde nekonecno a bod ma sednout na horni okraj ramecku.
            // Y() clamps at the bottom itself; the top is clamped here, so
            // past the sight limit the infinite value lands on the top edge.
            y: Math.max(AREA.y0, Y(required)),
            // Prazdny krouzek znamena "skutecny bod lezi az za okrajem
            // meritka" - plati pred obzorem (nula), pod spodni dekadou
            // i nad horni, kde je potrebna vyska porad konecna.
            // A hollow dot means the true point is off this scale: before the
            // horizon (zero), below the bottom decade, and above the top one,
            // where the required height is still finite.
            onScale: required >= yMin && required <= yMax,
            xLabel: F.distance(distance, lang),
            yLabel: F.distance(required, lang),
          };
        },
      });
    }
    ...
    function update(state, result, object) {
      ...
      const chart = svg('svg', { class: 'chart-svg rd-chart', xmlns: HL.dom.SVG_NS });
      container.appendChild(
